...
-(nonnull instancetype) initWithMetalDevice:(nonnull id<MTLDevice>)device renderDestinationProvider:(nonnull id<RenderDestinationProvider>)renderDestinationProvider {
    self = [super init];
    if(self) {
        _device = device;
...
- (void)_loadAssets {
    ...
    MDLMesh *mdlMesh = [MDLMesh newCylinderWithHeight:4 radii:(vector_float2){1.5, 1.5} radialSegments:30 verticalSegments:1 geometryType:MDLGeometryTypeTriangles inwardNormals:NO allocator:metalAllocator];
...
- (void)update {
    ...
    if (renderPassDescriptor != nil) {
        ...
        [renderEncoder pushDebugGroup:@"DrawCan"];
        ...
        [renderEncoder setVertexBuffer:_dynamicUniformBuffer offset:_uniformBufferOffset atIndex:kBufferIndexUniforms];

        [renderEncoder setFragmentBuffer:_dynamicUniformBuffer offset:_uniformBufferOffset atIndex:kBufferIndexUniforms];
