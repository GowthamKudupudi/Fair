...
class BasicNode {
   ...
   func render(commandQueue: MTLCommandQueue, 
      ...
      let uniformBuffer = bufferProvider.nextUniformsBuffer(
         projectionMatrix: projectionMatrix, viewMatrix: parentModelViewMatrix,
         modelMatrix: nodeModelMatrix, light: light
      )
      ...
      while i<coordinatesCount{
         ...
         if(i%3 == 0){
            ...
            if(i%9==0){
               ...
               var normal0 = triangle.flatMap{ $0 }.withUnsafeBufferPointer { 
                  (buffer) -> MyFloat3 in
...
func Curl(triangle:[[Float]])->[Float]{
   ...
   curl[2] = (triangle[1][0] - triangle[0][0])*(triangle[2][1]-triangle[0][1]) -
      (triangle[1][1] - triangle[0][1])*(triangle[2][0]-triangle[0][0]);
   curl[1] = (triangle[1][2] - triangle[0][2])*(triangle[2][1]-triangle[0][1]) -
      (triangle[1][0] - triangle[0][0])*(triangle[2][2]-triangle[0][0]);
   curl[0] = (triangle[1][1] - triangle[0][1])*(triangle[2][2]-triangle[0][2]) -
...
func Magnitude (of:float3) -> Float {
   var tmp:Double = (of.x^2) + (of.y^2) + (of.z^2)
   return sqrt(Float(tmp))
}
